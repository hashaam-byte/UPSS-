...
const EnhancedHeadAdminSidebar = ({ headAdmin }) => {
  const { sidebarOpen, sidebarCollapsed, toggleCollapse } = useSidebar();
  const { theme } = useTheme();

  const sidebarItems = [
    { icon: BarChart3, label: "Overview", active: true },
    { icon: School, label: "Manage Schools", active: false },
    { icon: CreditCard, label: "Premium Control", active: false },
    { icon: Award, label: "Guild of Scholars", active: false },
    { icon: Users, label: "User Management", active: false },
    { icon: DollarSign, label: "Financial Dashboard", active: false },
    ...
    <>
      ...
      >
          ...
          <AnimatePresence>
            {!sidebarCollapsed && (
              ...
              >
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">Head Admin Portal</p>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          <div className={`flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 ${sidebarCollapsed ? 'justify-center' : ''}`}>
            <motion.div 
              className="w-10 h-10 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
              whileHover={{ scale: 1.1, rotate: 5 }}
            >
              {headAdmin.avatar}
            </motion.div>
            ...
            <AnimatePresence>
              {!sidebarCollapsed && (
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                >
                  <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm">{headAdmin.name}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{headAdmin.title}</p>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <AnimatePresence>
        {sidebarOpen && (
          ...
          >
            ...
            {/* Mobile User Profile */}
            <div className="p-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20">
...
export default EnhancedHeadAdminSidebar;