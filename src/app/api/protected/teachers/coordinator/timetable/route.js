import { NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
import { cookies } from 'next/headers';
import jwt from 'jsonwebtoken';

const PERIODS = {
  "1": { start: "08:00", end: "09:05" },
  "2": { start: "09:05", end: "10:10" },
  "3": { start: "10:10", end: "11:15" }, 
  "BREAK": { start: "11:15", end: "11:35" },
  "4": { start: "11:35", end: "12:40" },
  "5": { start: "12:40", end: "13:45" },
  "LUNCH": { start: "13:45", end: "14:30" },
  "6": { start: "14:30", end: "15:35" }
};

const DAYS_OF_WEEK = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];

// Helper function to verify coordinator access
async function verifyCoordinatorAccess(token) {
  if (!token) {
    throw new Error('Unauthorized');
  }

  const decoded = jwt.verify(token, process.env.JWT_SECRET);
  const user = await prisma.user.findUnique({
    where: { id: decoded.userId },
    include: { teacherProfile: true, school: true }
  });

  if (!user || user.role !== 'teacher' || user.teacherProfile?.department !== 'coordinator') {
    throw new Error('Access denied');
  }

  return user;
}

// Helper function to check for teacher conflicts across all school timetables
async function checkTeacherConflicts(schoolId, teacherId, dayOfWeek, period, excludeEntryId = null) {
  const conflict = await prisma.timetable.findFirst({
    where: {
      schoolId,
      teacherId,
      dayOfWeek,
      period: parseInt(period),
      ...(excludeEntryId && { NOT: { id: excludeEntryId } })
    },
    include: {
      teacher: {
        select: { firstName: true, lastName: true }
      }
    }
  });

  if (conflict) {
    return {
      hasConflict: true,
      conflictDetails: {
        className: conflict.className,
        subject: conflict.subject,
        teacher: `${conflict.teacher.firstName} ${conflict.teacher.lastName}`,
        time: `${PERIODS[period].start} - ${PERIODS[period].end}`
      }
    };
  }

  return { hasConflict: false };
}

export async function GET(request) {
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get('auth_token')?.value;
    
    const coordinator = await verifyCoordinatorAccess(token);
    const { searchParams } = new URL(request.url);
    const className = searchParams.get('class');
    const dayOfWeek = searchParams.get('day');
    const teacherId = searchParams.get('teacher');
    const view = searchParams.get('view') || 'grid';

    // Get coordinator's assigned classes
    const coordinatorSubjects = await prisma.teacherSubject.findMany({
      where: {
        teacher: {
          user: { id: coordinator.id }
        }
      }
    });

    const coordinatorClasses = [...new Set(
      coordinatorSubjects.flatMap(ts => ts.classes)
    )];

    if (coordinatorClasses.length === 0) {
      return NextResponse.json({
        success: true,
        data: {
          timetable: {},
          message: 'No classes assigned to this coordinator'
        }
      });
    }

    // Build where clause - coordinator can only see their classes
    const whereClause = {
      schoolId: coordinator.schoolId,
      className: {
        in: coordinatorClasses
      },
      ...(className && { className }),
      ...(dayOfWeek && { dayOfWeek }),
      ...(teacherId && { teacherId })
    };

    // Get timetable entries with teacher details
    const timetable = await prisma.timetable.findMany({
      where: whereClause,
      include: {
        teacher: {
          select: {
            id: true,
            firstName: true,
            lastName: true,
            email: true,
            teacherProfile: {
              select: {
                department: true,
                qualification: true
              }
            }
          }
        },
        createdBy: {
          select: {
            firstName: true,
            lastName: true,
            teacherProfile: {
              select: { department: true }
            }
          }
        }
      },
      orderBy: [
        { className: 'asc' },
        { dayOfWeek: 'asc' },
        { period: 'asc' }
      ]
    });

    // Get available teachers for coordinator's classes
    const availableTeachers = await prisma.user.findMany({
      where: {
        schoolId: coordinator.schoolId,
        role: 'teacher',
        isActive: true,
        teacherProfile: {
          department: {
            in: ['subject_teacher', 'class_teacher']
          }
        }
      },
      select: {
        id: true,
        firstName: true,
        lastName: true,
        teacherProfile: {
          select: {
            department: true,
            teacherSubjects: {
              include: {
                subject: true
              }
            }
          }
        }
      },
      orderBy: [
        { firstName: 'asc' },
        { lastName: 'asc' }
      ]
    });

    // Get subjects available in the school
    const subjects = await prisma.subject.findMany({
      where: {
        schoolId: coordinator.schoolId,
        isActive: true,
        classes: {
          hasSome: coordinatorClasses
        }
      },
      include: {
        teachers: {
          include: {
            teacher: {
              include: {
                user: {
                  select: {
                    id: true,
                    firstName: true,
                    lastName: true
                  }
                }
              }
            }
          }
        }
      },
      orderBy: { name: 'asc' }
    });

    // Transform data based on view type
    let transformedData;
    
    if (view === 'grid') {
      // Grid view - organize by class, then by day and period
      transformedData = {};
      
      coordinatorClasses.forEach(cls => {
        transformedData[cls] = {};
        DAYS_OF_WEEK.forEach(day => {
          transformedData[cls][day] = {};
          Object.keys(PERIODS).forEach(periodNum => {
            if (periodNum !== 'BREAK' && periodNum !== 'LUNCH') {
              transformedData[cls][day][periodNum] = null;
            }
          });
        });
      });

      timetable.forEach(entry => {
        if (!transformedData[entry.className]) {
          transformedData[entry.className] = {};
        }
        if (!transformedData[entry.className][entry.dayOfWeek]) {
          transformedData[entry.className][entry.dayOfWeek] = {};
        }
        
        transformedData[entry.className][entry.dayOfWeek][entry.period] = {
          id: entry.id,
          subject: entry.subject,
          teacher: {
            id: entry.teacher.id,
            name: `${entry.teacher.firstName} ${entry.teacher.lastName}`,
            department: entry.teacher.teacherProfile?.department
          },
          startTime: entry.startTime,
          endTime: entry.endTime,
          createdBy: entry.createdBy ? {
            name: `${entry.createdBy.firstName} ${entry.createdBy.lastName}`,
            department: entry.createdBy.teacherProfile?.department
          } : null,
          createdAt: entry.createdAt,
          isOwnEntry: entry.createdById === coordinator.id
        };
      });
    } else {
      // List view
      transformedData = timetable.map(entry => ({
        id: entry.id,
        dayOfWeek: entry.dayOfWeek,
        period: entry.period,
        subject: entry.subject,
        className: entry.className,
        teacher: {
          id: entry.teacher.id,
          name: `${entry.teacher.firstName} ${entry.teacher.lastName}`,
          email: entry.teacher.email,
          department: entry.teacher.teacherProfile?.department
        },
        startTime: entry.startTime,
        endTime: entry.endTime,
        createdAt: entry.createdAt,
        createdBy: entry.createdBy ? `${entry.createdBy.firstName} ${entry.createdBy.lastName}` : null,
        isOwnEntry: entry.createdById === coordinator.id
      }));
    }

    // Calculate statistics
    const stats = {
      totalSlots: timetable.length,
      ownSlots: timetable.filter(t => t.createdById === coordinator.id).length,
      uniqueTeachers: [...new Set(timetable.map(t => t.teacherId))].length,
      uniqueSubjects: [...new Set(timetable.map(t => t.subject))].length,
      completionByClass: coordinatorClasses.map(cls => {
        const classSlots = timetable.filter(t => t.className === cls).length;
        const maxSlots = Object.keys(PERIODS).filter(p => p !== 'BREAK' && p !== 'LUNCH').length * DAYS_OF_WEEK.length;
        return {
          className: cls,
          slotsUsed: classSlots,
          totalPossibleSlots: maxSlots,
          completionRate: Math.round((classSlots / maxSlots) * 100)
        };
      })
    };

    return NextResponse.json({
      success: true,
      data: {
        timetable: transformedData,
        periods: PERIODS,
        daysOfWeek: DAYS_OF_WEEK,
        coordinatorClasses,
        availableTeachers: availableTeachers.map(t => ({
          id: t.id,
          name: `${t.firstName} ${t.lastName}`,
          department: t.teacherProfile?.department,
          subjects: t.teacherProfile?.teacherSubjects?.map(ts => ({
            name: ts.subject.name,
            code: ts.subject.code,
            classes: ts.classes
          })) || []
        })),
        subjects: subjects.map(s => ({
          id: s.id,
          name: s.name,
          code: s.code,
          category: s.category,
          classes: s.classes,
          assignedTeachers: s.teachers.map(ts => ({
            id: ts.teacher.user.id,
            name: `${ts.teacher.user.firstName} ${ts.teacher.user.lastName}`,
            classes: ts.classes
          }))
        })),
        statistics: stats,
        filters: {
          view,
          className,
          dayOfWeek,
          teacherId
        }
      }
    });

  } catch (error) {
    console.error('Coordinator timetable GET error:', error);
    
    if (error.message === 'Unauthorized') {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    if (error.message === 'Access denied') {
      return NextResponse.json({ error: 'Access denied' }, { status: 403 });
    }
    
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function POST(request) {
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get('auth_token')?.value;
    
    const coordinator = await verifyCoordinatorAccess(token);
    const { className, dayOfWeek, period, subject, teacherId, bulkEntries } = await request.json();

    // Get coordinator's assigned classes
    const coordinatorSubjects = await prisma.teacherSubject.findMany({
      where: {
        teacher: {
          user: { id: coordinator.id }
        }
      }
    });

    const coordinatorClasses = [...new Set(
      coordinatorSubjects.flatMap(ts => ts.classes)
    )];

    // Handle bulk creation
    if (bulkEntries && Array.isArray(bulkEntries)) {
      const results = {
        successful: [],
        failed: [],
        conflicts: [],
        suggestions: []
      };

      for (const entry of bulkEntries) {
        try {
          // Validate entry
          if (!entry.className || !entry.dayOfWeek || !entry.period || !entry.subject || !entry.teacherId) {
            results.failed.push({
              entry,
              error: 'Missing required fields'
            });
            continue;
          }

          // Check if coordinator has access to this class
          if (!coordinatorClasses.includes(entry.className)) {
            results.failed.push({
              entry,
              error: 'You do not have access to create timetable for this class'
            });
            continue;
          }

          // Check for class conflicts (same class, day, period)
          const classConflict = await prisma.timetable.findFirst({
            where: {
              schoolId: coordinator.schoolId,
              className: entry.className,
              dayOfWeek: entry.dayOfWeek,
              period: parseInt(entry.period)
            }
          });

          if (classConflict) {
            results.conflicts.push({
              entry,
              conflict: 'Time slot already occupied for this class',
              existing: {
                subject: classConflict.subject,
                teacher: classConflict.teacherId
              }
            });
            continue;
          }

          // Check for teacher conflicts across all school timetables
          const teacherConflictResult = await checkTeacherConflicts(
            coordinator.schoolId, 
            entry.teacherId, 
            entry.dayOfWeek, 
            entry.period
          );

          if (teacherConflictResult.hasConflict) {
            results.conflicts.push({
              entry,
              conflict: 'Teacher already has a class at this time',
              existing: teacherConflictResult.conflictDetails,
              suggestion: 'Consider assigning a different teacher or changing the time slot'
            });
            continue;
          }

          // Create entry
          const periodTimes = PERIODS[entry.period];
          const timetableEntry = await prisma.timetable.create({
            data: {
              schoolId: coordinator.schoolId,
              className: entry.className,
              dayOfWeek: entry.dayOfWeek,
              period: parseInt(entry.period),
              subject: entry.subject,
              teacherId: entry.teacherId,
              startTime: periodTimes.start,
              endTime: periodTimes.end,
              createdById: coordinator.id
            },
            include: {
              teacher: {
                select: {
                  firstName: true,
                  lastName: true
                }
              }
            }
          });

          results.successful.push({
            ...timetableEntry,
            teacher: {
              name: `${timetableEntry.teacher.firstName} ${timetableEntry.teacher.lastName}`
            }
          });

        } catch (error) {
          results.failed.push({
            entry,
            error: error.message
          });
        }
      }

      return NextResponse.json({
        success: true,
        message: `Bulk creation completed. ${results.successful.length} entries created successfully.`,
        data: results
      });
    }

    // Single entry creation
    if (!className || !dayOfWeek || !period || !subject || !teacherId) {
      return NextResponse.json({
        error: 'Missing required fields: className, dayOfWeek, period, subject, teacherId'
      }, { status: 400 });
    }

    // Check if coordinator has access to this class
    if (!coordinatorClasses.includes(className)) {
      return NextResponse.json({
        error: 'You do not have access to create timetable for this class'
      }, { status: 403 });
    }

    // Validate day of week
    if (!DAYS_OF_WEEK.includes(dayOfWeek)) {
      return NextResponse.json({
        error: `Invalid day of week. Must be one of: ${DAYS_OF_WEEK.join(', ')}`
      }, { status: 400 });
    }

    // Validate period
    const periodNum = parseInt(period);
    if (!PERIODS[periodNum] || period === 'BREAK' || period === 'LUNCH') {
      return NextResponse.json({
        error: `Invalid period. Must be one of: ${Object.keys(PERIODS).filter(p => p !== 'BREAK' && p !== 'LUNCH').join(', ')}`
      }, { status: 400 });
    }

    // Check for existing class timetable entry (class conflict)
    const existingClass = await prisma.timetable.findFirst({
      where: {
        schoolId: coordinator.schoolId,
        className,
        dayOfWeek,
        period: periodNum
      }
    });

    if (existingClass) {
      return NextResponse.json({
        error: 'Time slot already occupied for this class',
        suggestion: 'Please choose a different time slot'
      }, { status: 409 });
    }

    // Check for teacher availability across all school timetables
    const teacherConflictResult = await checkTeacherConflicts(
      coordinator.schoolId, 
      teacherId, 
      dayOfWeek, 
      periodNum
    );

    if (teacherConflictResult.hasConflict) {
      return NextResponse.json({
        error: 'Teacher already has a class at this time',
        conflict: teacherConflictResult.conflictDetails,
        suggestion: 'Consider assigning a different teacher or changing the time slot'
      }, { status: 409 });
    }

    // Verify teacher exists and belongs to school
    const teacher = await prisma.user.findFirst({
      where: {
        id: teacherId,
        schoolId: coordinator.schoolId,
        role: 'teacher',
        isActive: true
      }
    });

    if (!teacher) {
      return NextResponse.json({
        error: 'Teacher not found or invalid'
      }, { status: 400 });
    }

    // Create timetable entry
    const periodTimes = PERIODS[periodNum];
    const timetableEntry = await prisma.timetable.create({
      data: {
        schoolId: coordinator.schoolId,
        className,
        dayOfWeek,
        period: periodNum,
        subject,
        teacherId,
        startTime: periodTimes.start,
        endTime: periodTimes.end,
        createdById: coordinator.id
      },
      include: {
        teacher: {
          select: {
            firstName: true,
            lastName: true,
            email: true
          }
        }
      }
    });

    return NextResponse.json({
      success: true,
      message: 'Timetable entry created successfully',
      data: {
        id: timetableEntry.id,
        className: timetableEntry.className,
        dayOfWeek: timetableEntry.dayOfWeek,
        period: timetableEntry.period,
        subject: timetableEntry.subject,
        teacher: {
          id: timetableEntry.teacherId,
          name: `${timetableEntry.teacher.firstName} ${timetableEntry.teacher.lastName}`,
          email: timetableEntry.teacher.email
        },
        startTime: timetableEntry.startTime,
        endTime: timetableEntry.endTime,
        createdAt: timetableEntry.createdAt
      }
    }, { status: 201 });

  } catch (error) {
    console.error('Coordinator timetable POST error:', error);
    
    if (error.message === 'Unauthorized') {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    if (error.message === 'Access denied') {
      return NextResponse.json({ error: 'Access denied' }, { status: 403 });
    }
    
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function PUT(request) {
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get('auth_token')?.value;
    
    const coordinator = await verifyCoordinatorAccess(token);
    const { searchParams } = new URL(request.url);
    const entryId = searchParams.get('id');
    const { className, dayOfWeek, period, subject, teacherId } = await request.json();

    if (!entryId) {
      return NextResponse.json({ error: 'Entry ID is required' }, { status: 400 });
    }

    // Verify entry exists, belongs to school, and was created by this coordinator
    const existingEntry = await prisma.timetable.findFirst({
      where: {
        id: entryId,
        schoolId: coordinator.schoolId,
        createdById: coordinator.id // Coordinator can only edit their own entries
      }
    });

    if (!existingEntry) {
      return NextResponse.json({ 
        error: 'Timetable entry not found or you do not have permission to edit it' 
      }, { status: 404 });
    }

    // Get coordinator's assigned classes
    const coordinatorSubjects = await prisma.teacherSubject.findMany({
      where: {
        teacher: {
          user: { id: coordinator.id }
        }
      }
    });

    const coordinatorClasses = [...new Set(
      coordinatorSubjects.flatMap(ts => ts.classes)
    )];

    // Build update data
    const updateData = {};
    if (subject) updateData.subject = subject;
    if (teacherId) updateData.teacherId = teacherId;
    
    if (className) {
      if (!coordinatorClasses.includes(className)) {
        return NextResponse.json({
          error: 'You do not have access to assign this class'
        }, { status: 403 });
      }
      updateData.className = className;
    }
    
    if (dayOfWeek) updateData.dayOfWeek = dayOfWeek;
    
    if (period) {
      const periodNum = parseInt(period);
      updateData.period = periodNum;
      const periodTimes = PERIODS[periodNum];
      updateData.startTime = periodTimes.start;
      updateData.endTime = periodTimes.end;
    }

    // Check for conflicts if updating critical fields
    if (className || dayOfWeek || period || teacherId) {
      const checkData = {
        className: className || existingEntry.className,
        dayOfWeek: dayOfWeek || existingEntry.dayOfWeek,
        period: period ? parseInt(period) : existingEntry.period,
        teacherId: teacherId || existingEntry.teacherId
      };

      // Class conflict check (exclude current entry)
      const classConflict = await prisma.timetable.findFirst({
        where: {
          schoolId: coordinator.schoolId,
          className: checkData.className,
          dayOfWeek: checkData.dayOfWeek,
          period: checkData.period,
          NOT: { id: entryId }
        }
      });

      if (classConflict) {
        return NextResponse.json({
          error: 'Time slot already occupied for this class'
        }, { status: 409 });
      }

      // Teacher conflict check (exclude current entry)
      const teacherConflictResult = await checkTeacherConflicts(
        coordinator.schoolId,
        checkData.teacherId,
        checkData.dayOfWeek,
        checkData.period,
        entryId
      );

      if (teacherConflictResult.hasConflict) {
        return NextResponse.json({
          error: 'Teacher already has a class at this time',
          conflict: teacherConflictResult.conflictDetails
        }, { status: 409 });
      }
    }

    // Update entry
    const updatedEntry = await prisma.timetable.update({
      where: { id: entryId },
      data: updateData,
      include: {
        teacher: {
          select: {
            firstName: true,
            lastName: true,
            email: true
          }
        }
      }
    });

    return NextResponse.json({
      success: true,
      message: 'Timetable entry updated successfully',
      data: updatedEntry
    });

  } catch (error) {
    console.error('Coordinator timetable PUT error:', error);
    
    if (error.message === 'Unauthorized') {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    if (error.message === 'Access denied') {
      return NextResponse.json({ error: 'Access denied' }, { status: 403 });
    }
    
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function DELETE(request) {
  try {
    const cookieStore = await cookies();
    const token = cookieStore.get('auth_token')?.value;
    
    const coordinator = await verifyCoordinatorAccess(token);
    const { searchParams } = new URL(request.url);
    const entryId = searchParams.get('id');

    if (!entryId) {
      return NextResponse.json({ error: 'Entry ID is required' }, { status: 400 });
    }

    // Verify entry exists, belongs to school, and was created by this coordinator
    const existingEntry = await prisma.timetable.findFirst({
      where: {
        id: entryId,
        schoolId: coordinator.schoolId,
        createdById: coordinator.id // Coordinator can only delete their own entries
      }
    });

    if (!existingEntry) {
      return NextResponse.json({ 
        error: 'Timetable entry not found or you do not have permission to delete it' 
      }, { status: 404 });
    }

    // Delete entry
    await prisma.timetable.delete({
      where: { id: entryId }
    });

    return NextResponse.json({
      success: true,
      message: 'Timetable entry deleted successfully'
    });

  } catch (error) {
    console.error('Coordinator timetable DELETE error:', error);
    
    if (error.message === 'Unauthorized') {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    if (error.message === 'Access denied') {
      return NextResponse.json({ error: 'Access denied' }, { status: 403 });
    }
    
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}